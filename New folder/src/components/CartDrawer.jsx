...
export default function CartDrawer({ cart, onClose, onUpdateQty, onRemove }) {
  ...
  const total = subtotal + (freeShipping ? 0 : 500);
        ...
        <div className="cart-items">
          {cart.length === 0 ? (
            <div className="cart-empty">
              ...
              <p style={{ fontSize: 12, marginTop: 4 }}>฿3,000 minimum order required</p>
            </div>
          ) : (
            cart.map(item => (
              <div className="cart-item" key={item.id}>
                <div className="cart-item-img">{item.img && item.img.startsWith('/') ? <img src={item.img} alt={item.name} style={{ width: 50, height: 50, objectFit: 'cover', borderRadius: 6 }} /> : item.img}</div>
